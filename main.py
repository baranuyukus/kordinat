    ...
    def __do_request(self, post_data):
        try:
            ...
            if not hmac.compare_digest(client_computed, signature):
                QMessageBox.warning(self, "Error", "Signature checksum failed. Request was tampered with or session ended most likely.")
                return ""
            
            return response.text
        except requests.exceptions.Timeout:
            QMessageBox.warning(self, "Error", "Request timed out. Server is probably down/slow at the moment")
            return ""
...
class KeyAuthCircleApp(QWidget):
    ...
    def getchecksum(self):
        try:
            ...
            QMessageBox.warning(self, "Error", f"Failed to calculate checksum: {str(e)}")
            return ""
