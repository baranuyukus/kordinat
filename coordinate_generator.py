...
class CircleCoordinateGenerator(QWidget):
    ...
    def generate_concentric_circles(self, center_lat, center_long, max_radius, total_points):
        ...
        # Eğer toplam nokta sayısı 2000'den küçükse, tek bir çemberde yerleştirin
        if total_points <= 2000:
            num_circles = 1
            points_per_circle = total_points
        else:
        ...
        # Eğer kalan noktalar varsa, onları da ekleyin
        remaining_points = total_points % 2000
        if remaining_points > 0:
            circle_points = self.generate_circle_coordinates(center_lat, center_long, max_radius, remaining_points)
            points.extend(circle_points)
